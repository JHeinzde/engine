...
pub fn iterative_deepening(depth: u16, board: Board) -> (ChessMove, i32, Vec<ChessMove>) {
    let mut cache_table = CacheTable::new(33554432, TranspositionEntry { mov: None, lower_bound: None, upper_bound: None });
    ...
    for d in 1..depth + 1 {
        (first_guess, best_move, variation) = mtdf(board, first_guess, depth, &mut cache_table, maximize);
        //println!("{:?}", best_move);
    }
...
fn mtdf(board: Board, f: i32, depth: u16, cache_table: &mut CacheTable<TranspositionEntry>, maximize: bool) -> (i32, Option<ChessMove>, Vec<ChessMove>) {
    let mut score = f;
    let mut mov = None;
    ...
    while lower_bound < upper_bound {
        ...
        (score, mov, variation) = alpha_beta(depth, beta - 100, beta, maximize, board, cache_table);
        if score < beta {
            upper_bound = score;
        } else {
            lower_bound = score;
...
fn alpha_beta(depth: u16, mut alpha: i32, mut beta: i32, maximize: bool, board: Board, cache_table: &mut CacheTable<TranspositionEntry>) -> (i32, Option<ChessMove>, Vec<ChessMove>) {
    let entry = cache_table.get(board.get_hash());
    if entry != None {
    ...
    let mut move_gen = SortedMoveGen::new(move_gen, &board);
    ...
    if maximize {
        ...
        while (score < beta) && (mov != None) {
            let (tmp_score, _, move_stack) = alpha_beta(depth - 1, a, beta, false, board.make_move_new(mov.unwrap()), cache_table);
            if tmp_score > score {
                score = tmp_score;
        ...
        while (score > alpha) && (mov != None) {
            let (tmp_score, _, move_stack) = alpha_beta(depth - 1, alpha, b, true, board.make_move_new(mov.unwrap()), cache_table);
            if tmp_score < score {
                score = tmp_score;
                best_move = Some(mov.unwrap());
                mov_stack = move_stack;
    ...
    if score <= alpha {
        cache_table.add(board.get_hash(),
                        TranspositionEntry { mov: best_move, lower_bound: None, upper_bound: Some(score) })
    }
    if score > alpha && score < beta {
        cache_table.add(board.get_hash(),
                        TranspositionEntry { mov: best_move, lower_bound: Some(score), upper_bound: Some(score) })
    }
    if score >= beta {
        cache_table.add(board.get_hash(),
                        TranspositionEntry { mov: best_move, lower_bound: Some(score), upper_bound: None })
    }
